...
> **AI-powered news dashboard** — real-time fetching, Claude-driven analysis, and a polished interactive UI.

![Tech Stack](https://img.shields.io/badge/Backend-FastAPI-009688?logo=fastapi)
![Tech Stack](https://img.shields.io/badge/AI-Claude%20Haiku-6B45C4?logo=anthropic)
...
| 🤖 **AI Analysis** | Claude Haiku: 1-2 sentence summaries, sentiment (-1→+1), 3-5 key insights, keywords |
| ⚡ **Realtime Progress** | Server-Sent Events stream shows live pipeline progress in the browser |
...
4. `analyse` – concurrent Claude Haiku calls (5 at a time, semaphore-controlled)
5. `update` – write AI results back to DB; SSE pushes progress to browser
...
| `GET`  | `/api/pipeline/stream`        | SSE live progress stream |
...
| **Anthropic Claude Haiku** | Best cost-to-quality ratio for batch article analysis; same provider API for all AI tasks |
| **aiohttp + asyncio.Semaphore** | Concurrent pipeline without hammering external APIs — 5 parallel AI calls |
| **Server-Sent Events** | Lightweight one-way stream for pipeline progress; no WebSocket overhead |
...
- **Topic clustering** — group similar articles with embeddings (e.g. `text-embedding-3-small`)
